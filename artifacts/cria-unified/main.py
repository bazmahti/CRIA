...
class ThreeVoiceRenderer:
    ...
    async def _render_academic(self, cog, epi, conv, epi_academic, artefact):
        ...
        prompt = (
            ...
            f"Produce academic-voice output with these sections:\n"
            f"1. Abstract (200 words — NO mention of AI systems, models, or pipeline names)\n"
            f"2. Introduction (NO mention of AI systems, models, or pipeline names)\n"
            f"3. Methodology — use EXACTLY this formulation:\n"
            f"   'Evidence was gathered and synthesised using a multi-pipeline research "
            f"architecture applying parallel epistemic analysis across mainstream academic "
            f"databases (T1), grey literature and practitioner sources (T2), and advocacy "
            f"and counter-corpus material (T3). Position-privilege weighting was applied to "
            f"foreground sovereign and marginalised knowledge systems alongside credentialed "
            f"sources. Dissonance budget: {artefact.dissonance_budget}. "
            f"Counter-corpus retrieval: active.'\n"
            f"4. Findings (with evidence-tier transparency — NO AI, model, or pipeline names)\n"
            f"5. Discussion (position-privilege accounting, refusal signals — NO AI names)\n"
            f"6. Limitations\n"
            f"7. Conclusion (NO AI, model, or pipeline names)\n"
            f"8. References\n\n"
            f"AFTER the references, append this section verbatim:\n\n"
            f"---\n"
            f"RESEARCH INSTRUMENT NOTE\n\n"
            f"This research was conducted using a multi-pipeline AI-assisted research "
            f"architecture (CRIA — Civilisational Research Intelligence Architecture, "
            f"developed by Dr Barry Ferrier with Claude, Anthropic, 2025–2026). The "
            f"architecture applies parallel cognitive and epistemic analysis across 20 "
            f"channels with position-privilege weighting, frame-critical reading, and "
            f"counter-corpus retrieval. Full methodological documentation is available "
            f"on request. The findings, interpretations, and conclusions are the "
            f"researcher's own. The instrument is infrastructure; this paper is argument.\n"
            f"---\n\n"
        ...
        text = await call_llm(prompt, system_prompt=(
            ...
            "CRITICAL RULE: The abstract, introduction, findings, discussion, and conclusion "
            "must contain NO reference to AI systems, model names (Claude, GPT, Gemini, etc.), "
            "pipeline architecture, channel counts, or the name CRIA. These belong only in "
            "the designated methodology section and the appended disclaimer note. "
            "The paper must read as independent research scholarship, not an AI demonstration."
        ), max_tokens=4000)
    ...
    async def _render_editorial(self, cog, epi, conv, artefact):
        ...
        text = await call_llm(prompt, system_prompt=(
            ...
            "CRITICAL RULE: Include NO methodology section, NO disclaimer note, "
            "NO reference to AI systems, models, pipelines, channels, or research "
            "instrument of any kind. This output reads as journalism or public "
            "intellectual writing. The research instrument is entirely invisible."
        ), max_tokens=4000)
    ...
    async def _render_practitioner(self, cog, epi, conv, artefact):
        ...
        text = await call_llm(prompt, system_prompt=(
            ...
            "CRITICAL RULE: Include NO methodology section, NO disclaimer note, "
            "NO reference to AI systems, models, pipelines, channels, or research "
            "instrument of any kind. This output reads as direct professional "
            "guidance. The research instrument is entirely invisible."
        ), max_tokens=4000)
